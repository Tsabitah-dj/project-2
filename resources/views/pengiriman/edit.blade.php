            ...
            <div id="layoutAuthentication_content">
                ...
                <select name="karyawan_id" id="karyawan_id" class="form-select" required>
                    ...
                    @foreach($karyawan as $karyawan)
                        <option value="{{ $karyawan->id }}" {{ $karyawan->id == $pengiriman->karyawan_id ? 'selected' : '' }}>{{ $karyawan->nama_karyawan }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select name="barang_id" id="barang_id" class="form-select" required>
                    ...
                    @foreach($barang as $barang)
                        <option value="{{ $barang->id }}" {{ $barang->id == $pengiriman->barang_id ? 'selected' : '' }}>{{ $barang->nama_barang }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                <label for="jumlah" class="form-label">Jumlah Barang</label>
                <input type="number" name="jumlah" id="jumlah" class="form-control" value="{{ $pengiriman->jumlah }}" required />
            </div>
            <div class="mb-3">
                <label for="alamat" class="form-label">Alamat</label>
                <input type="text" name="alamat" id="alamat" class="form-control" value="{{ $pengiriman->alamat }}" required />
            </div>
